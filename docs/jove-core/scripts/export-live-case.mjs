...
if (!args["task-id"] || !args["case-id"] || !args["task-class"] || !args["consent-at"]) {
  throw new Error("Required: --task-id=... --case-id=jove_core_0001 --task-class=... --consent-at=<ISO timestamp>");
}
...
const record = {
  ...
  consent: { research_use: true, recorded_at: new Date(args["consent-at"]).toISOString(), retention_policy: String(args.retention || "Private source evidence retained for the approved study period; public release is metadata-only.") },
