...
MOCK_DELIVERIES = {
    "ORD-1001": {"status": "delivered", "delivered_at": "2026-08-20T12:00:00Z", "order_id": "ORD-1001"},
...
def check_delivery(reference_id: str) -> dict:
    ...
    return {"found": True, "status": record["status"], "delivered_at": record["delivered_at"], "order_id": record["order_id"]}
...
def run_delivery_verification(evidence: dict) -> dict:
    ...
    checks.append({
        ...
        "detail": f"order {ref} captured at {captured_at}" if ref and captured_at else "referenceId and capturedAt are required",
    })
    checks.append({
        "name": "integrity",
    ...
    if delivery["found"]:
        if delivery["status"] == "delivered":
            try:
                ...
                detail = "order %s delivered, capture within 1h" % ref if consistency_ok else "capture time far from delivery time"
            except ValueError:
    ...
    checks.append({
        "name": "judgment",
        "passed": gps_ok,
        "detail": f"gps {gps} plausible" if gps_ok else "gps coordinates implausible or missing",
    })
